...
export default async function SdmPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">SDM — Sumber Daya Manusia</h1>
        <p className="text-sm text-gray-500 mt-0.5">Manajemen karyawan semua unit bisnis</p>
      </div>

      {/* Unit summary */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {unitTypeCounts.map((u) => (
          <div key={u.id} className="bg-white rounded-xl border border-gray-100 shadow-sm px-5 py-4">
            <p className="text-xs text-gray-500 font-medium truncate">{u.name}</p>
            <p className="text-2xl font-bold text-gray-800 mt-1">{u.count}</p>
